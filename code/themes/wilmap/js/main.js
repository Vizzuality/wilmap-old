    ...
    attach: function attach(context, settings) {
      ...
      if ($(context).find('.news-page').length !== 0) {
        ...
        $.ajax({
          ...
          success: function success(data, status, xhr) {
            for (var i = 0; i < data.length; i++) {
              var contentbox = '<div class="info-news"><h2>' + data[i].title + '</h2><span class="date">' + data[i].field_date_published + '</span><div class="text">' + data[i].field_summary + '</div><a class="butn -primary" href="' + data[i].path + '">read more</a></div>';
